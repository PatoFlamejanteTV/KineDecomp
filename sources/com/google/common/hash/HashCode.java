...
public abstract class HashCode {
    ...
    /* renamed from: a */
    private static final char[] f15939a = "0123456789abcdef".toCharArray();
    ...
    public static final class BytesHashCode extends HashCode implements Serializable {
        ...
        @Override // com.google.common.hash.HashCode
        public int asInt() {
            Preconditions.b(this.bytes.length >= 4, "HashCode#asInt() requires >= 4 bytes (it only has %s bytes).", this.bytes.length);
        ...
        @Override // com.google.common.hash.HashCode
        public long asLong() {
            Preconditions.b(this.bytes.length >= 8, "HashCode#asLong() requires >= 8 bytes (it only has %s bytes).", this.bytes.length);
            return padToLong();
        }
    ...
    public final String toString() {
        ...
        for (byte b2 : bytesInternal) {
            sb.append(f15939a[(b2 >> 4) & 15]);
            sb.append(f15939a[b2 & ap.m]);
        }
        return sb.toString();
    }
    ...
    public int writeBytesTo(byte[] bArr, int i, int i2) {
        int a2 = Ints.a(i2, bits() / 8);
        Preconditions.a(i, i + a2, bArr.length);
        writeBytesToImpl(bArr, i, a2);
        return a2;
    }
