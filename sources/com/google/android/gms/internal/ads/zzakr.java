...
public final class zzakr {
    ...
    public zzakr(JSONObject jSONObject) throws JSONException {
        ...
        for (int i2 = 0; i2 < jSONArray.length(); i2++) {
            try {
                zzakq zzakqVar = new zzakq(jSONArray.getJSONObject(i2));
                boolean z = true;
                if (zzakqVar.zzuj()) {
                    this.zzdmi = true;
                }
                ...
                if (i < 0) {
                    Iterator<String> it = zzakqVar.zzdkw.iterator();
                    while (true) {
                        if (it.hasNext()) {
        ...
        this.zzdmb = jSONObject.optLong("timeout_ms", -1L);
        JSONObject optJSONObject = jSONObject.optJSONObject("settings");
        if (optJSONObject != null) {
            this.zzdlo = optJSONObject.optLong("ad_network_timeout_millis", -1L);
