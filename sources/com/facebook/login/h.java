...
abstract class H extends D {
    ...
    public Bundle getParameters(LoginClient.Request request) {
        ...
        if (token != null && token.equals(loadCookieToken())) {
            ...
            Utility.clearFacebookCookies(this.loginClient.c());
    ...
    public void onComplete(LoginClient.Request request, Bundle bundle, FacebookException facebookException) {
        ...
        if (bundle != null) {
            ...
            try {
                AccessToken createAccessTokenFromWebBundle = D.createAccessTokenFromWebBundle(request.getPermissions(), bundle, getTokenSource(), request.getApplicationId());
                createErrorResult = LoginClient.Result.createTokenResult(this.loginClient.i(), createAccessTokenFromWebBundle);
                CookieSyncManager.createInstance(this.loginClient.c()).sync();
                saveCookieToken(createAccessTokenFromWebBundle.getToken());
            } catch (FacebookException e2) {
                createErrorResult = LoginClient.Result.createErrorResult(this.loginClient.i(), null, e2.getMessage());
            }
        } else if (facebookException instanceof FacebookOperationCanceledException) {
            createErrorResult = LoginClient.Result.createCancelResult(this.loginClient.i(), "User canceled log in.");
        } else {
            this.e2e = null;
            ...
            createErrorResult = LoginClient.Result.createErrorResult(this.loginClient.i(), null, message, str);
        }
        if (!Utility.isNullOrEmpty(this.e2e)) {
            logWebLoginCompleted(this.e2e);
        }
        this.loginClient.b(createErrorResult);
    }
